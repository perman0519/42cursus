...
/*   get_next_line.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: junssong <junssong@studen.42seoul.>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/03 12:04:32 by junssong          #+#    #+#             */
/*   Updated: 2023/04/03 12:04:32 by junssong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	ft_memset(new_str, 0, BUFFER_SIZE + 1);
...
int	is_same_readsize(t_fd_list *fd_list, char *new_str,
		char **return_str, ssize_t *i)
{
	*return_str = ft_strjoin(*return_str, new_str);
	fd_list->index = 0;
	free(fd_list->buf);
	if (!(*return_str))
		return (0);
