...
/*   Updated: 2024/03/24 13:27:22 by junssong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_split_line(char **split, t_info *info)
{
	...
	else if (ft_strlen(split[0]) == 2 && !ft_strncmp(split[0], "pl", 2))
		return (pl_init(split, info));
	else if (ft_strlen(split[0]) == 2 && !ft_strncmp(split[0], "sp", 2))
		return (sp_init(split, info));
	else if (ft_strlen(split[0]) == 3 && !ft_strncmp(split[0], "nsp", 3))
		return (nsp_init(split, info));
	else if (ft_strlen(split[0]) == 2 && !ft_strncmp(split[0], "cy", 2))
		return (cy_init(split, info));
	else if (ft_strlen(split[0]) == 2 && !ft_strncmp(split[0], "pb", 2))
		return (pb_init(split, info));
